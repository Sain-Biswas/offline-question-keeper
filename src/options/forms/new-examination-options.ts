...
export const newExaminationSchema = z.object({
	name: z.string().min(1, { error: "Examination name is required." }).min(3, {
		error: "Examination name must be at least 3 characters long."
	}),
	code: z.string().min(1, { error: "Examination code is required." }).min(2, {
		error: "Examination code must be at least 2 characters long."
	}),
	slug: z
		.string()
	...
	description: z.string().trim().max(250, {
		error: "Description can be at most 250 characters long."
	})
});
